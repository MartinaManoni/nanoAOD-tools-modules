    ...
    def analyze(self, event):
        ...
        for jet in jets:
            ...
            if self.is_mc:
                ...
                eventNumber  = int(event.event & 0x7FFFFFFF)
                JERsmear = self.evaluator_JERsmear.evaluate(pt_JEC, jet.eta, pt_gen, event.Rho_fixedGridRhoFastjetAll, eventNumber, JER, JERsf)
                JERsmear_up = self.evaluator_JERsmear.evaluate(pt_JEC, jet.eta, pt_gen, event.Rho_fixedGridRhoFastjetAll, eventNumber, JER, JERsf_up)
                JERsmear_dn = self.evaluator_JERsmear.evaluate(pt_JEC, jet.eta, pt_gen, event.Rho_fixedGridRhoFastjetAll, eventNumber, JER, JERsf_dn)
